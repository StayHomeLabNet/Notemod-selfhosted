...
$t = [
  'ja' => [
    ...
    'secret_note' => 'SECRET が config/config.php に無い場合、自動で追記します（既にあれば変更しません）',
    ...
    'api_note' => '空で保存すると、その項目は更新しません（既存値は維持）',
  ...
  'en' => [
    ...
    'secret_note' => 'If SECRET is missing in config/config.php, it will be appended automatically (won’t overwrite existing).',
    ...
    'api_note' => 'If left empty, it won’t update that field (existing values kept).',
...
function nm_update_config_api_tokens_preserve(string $configApiPath, string $expectedToken, string $adminToken): bool
{
    ...
    if (!file_exists($configApiPath)) {
        $tpl = "<?php\n"
             ...
             . "// EXPECTED_TOKEN は、通常のAPIトークン\n"
             . "// ADMIN_TOKEN は、cleanup専用の強いトークン\n"
             . "return [\n"
             . "    'EXPECTED_TOKEN' => " . var_export($expectedToken !== '' ? $expectedToken : 'CHANGE_ME', true) . ", // ★必ず変更\n"
             . "    'ADMIN_TOKEN' => " . var_export($adminToken !== '' ? $adminToken : 'CHANGE_ME', true) . ", // ★必ず変更\n\n"
             . "    // api.php から見た data.json の絶対パス\n"
...
try {
    ...
    $prefExpected = (string)($cfgApi['EXPECTED_TOKEN'] ?? '');
    $prefAdmin    = (string)($cfgApi['ADMIN_TOKEN'] ?? '');
} catch (Throwable $e) {
    // ignore
}
        ...
        elseif (strlen($p1) < 10) $err = $t[$lang]['err_pw_short'];
            ...
            if (!$h) $err = $t[$lang]['err_write_auth'];
                ...
                if (!nm_auth_write_config($u, $h)) $err = $t[$lang]['err_write_auth'];
                    ...
                    $already = true;
                    // 初回作成直後でも canEditTokens は true のまま（初回セットアップなのでOK）
                }
            }
        }
    }
